...
export async function demoHandle(method: string, path: string, body: Record<string, unknown> = {}): Promise<any> {
  ...
  if (p === "/api/invoices" && method === "GET") {
    ...
    const enriched = list.map((inv) => { const b = db.Bookings.find((x) => x.bookingId === inv.bookingId) || {} as Row; return { ...inv, serviceName: svcById(b.serviceId)?.name || "", recipientName: rcpById(b.recipientId || inv.recipientId)?.name || "", householdName: hhById(inv.householdId)?.name || "" } as Row; }).sort((a, b) => ((a as Row).createdAt < (b as Row).createdAt ? 1 : -1));
    return { invoices: enriched };
  }
  if (p === "/api/invoices" && method === "POST") {
    if (body.action === "create") { const items = Array.isArray(body.lineItems) ? (body.lineItems as Row[]) : []; const total = items.reduce((s, it) => s + (parseFloat(String(it.amount)) || 0), 0) || (parseFloat(String(body.amount)) || 0); const number = String(body.number || ("INV-" + String(db.Invoices.length + 1).padStart(4, "0"))); db.Invoices.push({ invoiceId: id("inv"), tenantId: IDS.tenant, householdId: String(body.householdId || ""), recipientId: String(body.recipientId || ""), bookingId: "", amount: (Math.round(total * 100) / 100).toFixed(2), status: "unpaid", stripeId: "", lineItems: items, notes: String(body.notes || ""), dueDate: String(body.dueDate || ""), template: String(body.template || "clean"), number, createdAt: now() } as any); write(db); return { ok: true, number }; }
    if (body.action === "update") { const inv = db.Invoices.find((i) => i.invoiceId === body.invoiceId); if (inv) { if (Array.isArray(body.lineItems)) { (inv as any).lineItems = body.lineItems; inv.amount = ((body.lineItems as Row[]).reduce((s, it) => s + (parseFloat(String(it.amount)) || 0), 0)).toFixed(2); } for (const k of ["notes", "dueDate", "template", "householdId", "recipientId", "number"]) if (k in body) (inv as Row)[k] = String((body as Row)[k]); } write(db); return ok(); }
